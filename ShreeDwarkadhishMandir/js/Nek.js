...
function ApplicationUser() {
    $.ajax({
        url: "/ApplicationUser/ApplicationUserDropdown",
        dataType: "json",
        type: "POST",
        ...
        success: function (jsondata) {
            ...
            if (typeof jsondata !== "undefined") {
                $(jsondata).each(function () {
                    if (this.DisplayName !== 'Vallabhacharya' && this.DisplayName !== 'System Admin') {
                        $("<option />", {
                            val: this.Id,
                            text: this.DisplayName + ' - ' + this.UserTypeName
                        }).appendTo("#NekUser");
                    }
                });
            }
...
function SaveForm() {
    showProgress();
    var NekRequest = {};
    NekRequest.Description = $("#Description").val();
    NekRequest.ItemDetails = transactionDetails;
    NekRequest.ApplicationUser = parseInt($("#NekUser").val());
    $.ajax({
        url: "/SamagriTransaction/Nek",
        data: JSON.stringify(NekRequest),
        dataType: "json",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        success: function (jsondata) {
            alert("Nek has been saved successfully.");
            hideProgress();
...
function GetStoreList() {
    $.ajax({
        ...
        success: function (jsondata) {
            $("#Store").empty();
            var prasadGhar = jsondata.filter(function (i, n) { return (i.StoreType === 3 || i.StoreType === 2) && i.IsActive === true });
