...
function SaveForm() {
    ...
    $.ajax({
        ...
        success: function (jsondata) {
            alert("Bhandar has been Issue or transfer successfully.");
            hideProgress();
...
function SaveItem() {
    ...
    $.map(transactionDetails, function (e, i) {
        if (e.BhandarId == BhandarId && e.StoreToId == StoreToId) {
            added = true;
        }
    });
    if (!added) {
        ...
        purchaseDetail.StoreToId = StoreToId;
        purchaseDetail.StoreToName = StoreToId > 0 ? $('#StoreTo').find("option:selected").text() : '';
