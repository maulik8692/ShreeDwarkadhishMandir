...
BEGIN                
 ...
 BT.ReceiptId=MR.Id  
 where isnull(@ReceiptNo,'')='' or (
 isnull(MR.ManorathiName,'Vaishnav') like @ReceiptNo+'%'
 or 
 (case when ISNUMERIC(MR.Description)=1 then MR.Description else MR.ReceiptNo end) like @ReceiptNo+'%'
 or 
 M.ManorathName like @ReceiptNo+'%')
